...
Each file contains:
- **Column 1:** `Timestamp` (in seconds) – consistent sampling across all files  
- **Column 2:** `Voltage` (in volts) – generated by the TENG sensor in real-time during the activity
...
- Runs `train_v5.py` 5 times with different random seeds (0–4).
...
## 🧪 Subject Generalization: LOSO Results
...
## 🔬 Discussion: Techniques in train_v5.py vs train_v6.py
...
- **No Data Augmentation:**
  - For a fair LOSO test, no artificial augmentation is applied—only real, subject-specific segments are used.
- **Model:**
