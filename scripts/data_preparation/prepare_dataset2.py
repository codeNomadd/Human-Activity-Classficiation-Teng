...
for fname, label in file_label_map.items():
    ...
    # Split into 10 non-overlapping 500-point segments
...
X = np.stack(X)  # shape (90, 500)
y = np.array(y)  # shape (90,)

# Save as .npy files for easy loading later
np.save(os.path.join(DATA_DIR, 'X_500.npy'), X)
np.save(os.path.join(DATA_DIR, 'y_500.npy'), y)
